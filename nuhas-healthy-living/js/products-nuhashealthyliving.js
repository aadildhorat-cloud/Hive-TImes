/**
🌿 Nuhas Healthy Living - Centralized Product Data & Utilities (ULTRA PERFORMANCE EDITION)
📁 Path: /nuhas-healthy-living/js/products-nuhashealthyliving.js
✅ FIXED: Removed all syntax errors, fixed template literals, and exposed window.NUHAS_HEALTHY_LIVING_PRODUCTS.
*/
(function () {
'use strict';
...
SHEETS_API_URL: "https://script.google.com/macros/s/AKfycbwWOuBgx0vcrWaAU3EpTMa27LRRtyJoBtILNwTgtMX-E1fxMCIwdxl2QWY5T60qW3s/exec",
basePath: "/nuhas-healthy-living",
imageDir: "/images2",
...
CACHE_KEY: "nuhashealthyliving_products_cache",
CART_KEY: "nuhashealthyliving_cart",
...
{ id: "kulunji-oil-capsules", name: "Kulonji Oil Capsules", price: 275.00, category: "supplements-capsules", niche: "medication", location: "azadville", description: "Fights cancer, promotes liver health, combats diabetes, aids weight loss.", badge: "✨ Best Seller", image: "/nuhas-healthy-living/images2/1.jpg" },
{ id: "marine-collagen-peptides", name: "Hydrolysed Marine Collagen Peptides (Type I)", price: 275.00, category: "supplements-capsules", niche: "medication", location: "azadville", description: "Anti-ageing & joint support, repairs skin/hair/nails, reduces wrinkles.", badge: "✨ Premium", image: "/nuhas-healthy-living/images2/2.jpg" },
{ id: "organic-baby-powder", name: "Organic Baby Dusting Powder", price: 200.00, category: "baby-personal", niche: "medication", location: "azadville", description: "Talc-free, prevents nappy rash, absorbs moisture, deodorizes, protects delicate skin.", badge: "👶 Baby", image: "/nuhas-healthy-living/images2/36.jpg" }
];
// 🌐 State management
let PRODUCTS = [];
...
console.log('⚡ Loaded Nuhas Healthy Living products from cache (instant)');
...
return PRODUCTS;
     ...
     if (!CONFIG.SHEETS_API_URL || CONFIG.SHEETS_API_URL === "" || CONFIG.SHEETS_API_URL.includes("YOUR_NUHASHEALTHYLIVING_DEPLOYMENT_ID")) {
...
if (!CONFIG.SHEETS_API_URL || CONFIG.SHEETS_API_URL === "" || CONFIG.SHEETS_API_URL.includes("YOUR_NUHASHEALTHYLIVING_DEPLOYMENT_ID")) return;
try {
...
if (!productsArray) return;
     ...
     if (lastRawSnapshot === null) lastRawSnapshot = JSON.stringify(window.NUHASHEALTHYLIVING_INLINE_PRODUCTS || window.NUHAS_HEALTHY_LIVING_PRODUCTS || []);
...
niche: (product.niche || "health-beauty").trim(),
location: (product.location || "azadville").trim(),
...
nicheSlug: (product.niche || "health-beauty").trim().toLowerCase().replace(/\s+/g, '-'),
locationSlug: (product.location || "azadville").trim().toLowerCase().replace(/\s+/g, '-')
};
PRODUCTS_MAP.set(processed.id, processed);
return processed;
});
 // ✅ CRITICAL FIX: Expose to window.NUHAS_HEALTHY_LIVING_PRODUCTS so index.html can render them!
 window.NUHAS_HEALTHY_LIVING_PRODUCTS = PRODUCTS;
 window.NUHASHEALTHYLIVING_DATA = PRODUCTS;
 window.HIVETIMES_NUHASHEALTHYLIVING_PRODUCTS = PRODUCTS;
 return PRODUCTS;
}
...
this.save();
this.updateUI();
this.dispatchEvent('nuhashealthyliving:cart:updated', { cart: this.items, addedProduct: product });
return true;
},
...
this.save();
this.updateUI();
this.dispatchEvent('nuhashealthyliving:cart:updated', { cart: this.items });
return true;
},
...
// 🛠️ Utility API
window.NuhasHealthyLivingProducts = {
getAll: () => PRODUCTS,
...
return `<article class="product-card" data-id="${p.id}" data-category="${p.categorySlug}" data-price="${p.price}" data-name="${p.name}" data-description="${p.description}" data-image="${p.image}" onclick="openModal('${p.id}')" role="button" tabindex="0" style="cursor:pointer;"> <div class="product-image-wrap"> <img src="${p.image}" alt="${p.name}" loading="lazy" decoding="async" class="product-image" onerror="this.src='${p.imageFallback}'"> ${p.badge ? `<span class="product-badge">${p.badge}</span>` : ''} </div> <div class="product-info"> <h3 class="product-name">${p.name}</h3> <p class="product-description">${p.description}</p> <div class="product-price">${priceDisplay}</div> <button class="add-to-cart-btn" onclick="event.stopPropagation(); NuhasHealthyLivingProducts.addToCart('${p.id}'); return false;"> ${btnText} </button> </div> </article>`;
},
...
else document.dispatchEvent(new CustomEvent('nuhashealthyliving:modal:open', { detail: { productId } }));
},
getStatus: () => ({
...
window.NuhasHealthyLivingCart = Cart;
// 🚀 INITIALIZATION
(async function init() {
Cart.init();
 // ✅ FIXED: Check for both NUHASHEALTHYLIVING_INLINE_PRODUCTS and NUHAS_HEALTHY_LIVING_PRODUCTS
  const inlineData = window.NUHASHEALTHYLIVING_INLINE_PRODUCTS || window.NUHAS_HEALTHY_LIVING_PRODUCTS;
  ...
  if (hasInlineData) {
      ...
      try {
          document.dispatchEvent(new CustomEvent('nuhashealthyliving:products:loaded', { detail: { products: PRODUCTS } }));
      } catch (err) {}
      setTimeout(() => backgroundRefresh(window.NUHASHEALTHYLIVING_INLINE_PRODUCTS_HASH), 1500);
  } else {
      await fetchProducts();
      try {
          document.dispatchEvent(new CustomEvent('nuhashealthyliving:products:loaded', { detail: { products: PRODUCTS } }));
  ...
  console.group('🌿 Nuhas Healthy Living Products Initialized');
...
const productList = {
 ...
 "@type": "ItemList",
 "name": "Nuhas Healthy Living Product Catalog",
 "description": "100% Organic, Halaal-certified supplements, personal care, and wellness products from Nuhas Healthy Living.",
 ...
 "image": product.image.startsWith('http') ? product.image : `https://nuhashealthyliving.co.za/${product.image}`,
 "sku": product.id,
 "brand": { "@type": "Brand", "name": "Nuhas Healthy Living" },
 "offers": {
 "@type": "Offer",
 "priceCurrency": "ZAR",
 ...
 "seller": { "@type": "Organization", "name": "Nuhas Healthy Living" }
}
}
}))
};
 let schemaEl = document.getElementById('nuhashealthylivingProductSchema');
 if (!schemaEl) {
     schemaEl = document.createElement('script');
     schemaEl.type = 'application/ld+json';
     schemaEl.id = 'nuhashealthylivingProductSchema';
     document.head.appendChild(schemaEl);
 }
 schemaEl.textContent = JSON.stringify(productList);
}
document.addEventListener('nuhashealthyliving:products:loaded', () => setTimeout(generateProductSchema, 500));
